...
const InfinityUpgradesIDGrid = [
  ["timeMult", "dimMult", "timeMult2", "skipReset1"],
  ["18Mult", "27Mult", "unspentBonus", "skipReset2"],
  ["36Mult", "45Mult", "resetMult", "skipReset3"],
  ["resetBoost", "galaxyBoost", "passiveGen", "skipResetGalaxy"],
];

const InfinityUpgradesGrid = [
  ["totalTimeMult", "buy10Mult", "thisInfinityTimeMult", "skipReset1"],
  ["dim18mult", "dim27mult", "unspentIPMult", "skipReset2"],
  ["dim36mult", "dim45mult", "dimboostMult", "skipReset3"],
  ["resetBoost", "galaxyBoost", "ipGen", "skipResetGalaxy"],
];

function isPositionVaild(position) {
  if (position.filter((x) => x < 0 || x > 3).length) return false;
  return true;
}

function getUpgradesAround(id) {
  let upgradePosition = [null, null];
  for (let x in InfinityUpgradesIDGrid) {
    for (let y in InfinityUpgradesIDGrid[x]) {
      if (InfinityUpgradesIDGrid[x][y] === id) upgradePosition = [Number(x), Number(y)];
    }
  }
  console.log("original", id, upgradePosition)
  const directions = [
    [1, 0],
    [-1, 0],
    [0, 1],
  ...
  for (let direction of directions) {
    let position = [upgradePosition[0] + direction[0], upgradePosition[1] + direction[1]];
    console.log(position, isPositionVaild(position))
    if (!isPositionVaild(position)) continue;
    vaildUpgrades.push(InfinityUpgradesGrid[position[0]][position[1]]);
  }
  return vaildUpgrades;
}
...
export class InfinityUpgradeState extends SetPurchasableMechanicState {
  ...
  purchase() {
    if (super.purchase()) {
      ...
      console.log("upgrades around", getUpgradesAround(this.id))

      for (let upgrade of getUpgradesAround(this.id)) {
        InfinityUpgrade[upgrade].toggle()
      }
...
export function totalIPMult() {
  ...
  let ipMult = DC.D1.times(ShopPurchase.IPPurchases.currentMult).timesEffectsOf(
    TimeStudy(41),
    TimeStudy(51),
    TimeStudy(141),
    TimeStudy(142),
