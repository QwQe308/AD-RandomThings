...
export function buyMaxDimension(tier, bulk = Infinity) {
  ...
  if (dimension.currencyAmount.gte(cost) || dimension.currencyAmount.neg().gte(dimension.cost)) {
    dimension.currencyAmount = dimension.currencyAmount.minus(cost);
    buyUntilTen(tier);
    bulkLeft--;
  }
  ...
  const maxBought = dimension.costScale.getMaxBought(
    Math.floor(dimension.bought / 10) + dimension.costBumps, dimension.currencyAmount.abs(), 10
  );
  if (maxBought === null) {
    return;
  }
...
class AntimatterDimensionState extends DimensionState {
  ...
  get howManyCanBuy() {
    ...
    if(this.currencyAmount.lt(0) && this.currencyAmount.neg().gt(this.cost)) ratio = 10
   ...
   */
  get totalAmount() {
    if (!Laitela.continuumActive) return this.amount;
  ...
  get isAffordable() {
    ...
    return this.cost.lte(this.currencyAmount.abs());
  }
  ...
  get isAffordableUntil10() {
    if (!player.break && this.cost.gt(Decimal.NUMBER_MAX_VALUE)) return false;
    if(this.currencyAmount.lt(0)) return this.cost.lte(this.currencyAmount.neg())
  ...
  get productionPerSecond() {
    ...
    if (NormalChallenge(12).isRunning) {
      if (tier === 2) amount = amount.abs().pow(0.6).mul(amount);
      if (tier === 4) amount = amount.abs().pow(0.4).mul(amount);
      if (tier === 6) amount = amount.abs().pow(0.2).mul(amount);
    }
    ...
    if (tier === 1) {
      ...
      if (production.gt(10)) {
        const log10 = production.abs().log10();
        production = Decimal.pow10(Math.pow(log10, getAdjustedGlyphEffect("effarigantimatter"))).mul(production.lt(0) ? -1 : 1);
      }
    }
    if(!player.break) production = production.min(this.cappedProductionInNormalChallenges);
    if(player.break) production = production.max(this.cappedProductionInNormalChallenges.neg());
    return production;
  }
}
