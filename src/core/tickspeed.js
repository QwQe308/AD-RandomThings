...
export function effectiveBaseGalaxies() {
  ...
  return Math.max(player.galaxies + GalaxyGenerator.galaxies + replicantiGalaxies + freeGalaxies + 1, 0);
}
...
export function buyMaxTickSpeed() {
  ...
  if (NormalChallenge(9).isRunning) {
    ...
    while (Currency.antimatter.abs().gt(cost) && cost.lt(goal)) {
      Tickspeed.multiplySameCosts();
    ...
    const purchases = Tickspeed.costScale.getMaxBought(player.totalTickBought, Currency.antimatter.abs(), 1);
    if (purchases === null) {
      return;
    }
...
export const Tickspeed = {
  ...
  get isAffordable() {
    return Currency.antimatter.abs().gte(this.cost);
  },
