...
export function gainedInfinityPoints() {
  ...
  let ip = player.break
    ...
    : Decimal.pow10(player.records.thisInfinity.minAM.abs().log10() / div - 0.75);
...
function passivePrestigeGen() {
  ...
  if (!EternityChallenge(4).isRunning) {
    ...
    if (BreakInfinityUpgrade.infinitiedGen.isBought) {
      ...
      infGen = infGen.plus(1.5 * Time.deltaTimeMs / Math.clampMin(50, player.records.bestInfinity.time));
